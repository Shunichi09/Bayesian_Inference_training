...
class GibsSampler():
    ...
    def sample(self):
        ...
        """
        # sampling x1
        mean = self.true_mean[0] -  1./self.true_lam[0, 0] * self.true_lam[0, 1] * (self.points_x2[-1] - self.true_mean[1])
        point_x1 = np.random.normal(mean, 1./self.true_lam[0, 0])

        self.points_x1.append(point_x1)

        # sampling x2
        mean = self.true_mean[1] -  1./self.true_lam[1, 1] * self.true_lam[1, 0] * (self.points_x1[-1] - self.true_mean[0])
        point_x2 = np.random.normal(mean, 1./self.true_lam[1, 1])
